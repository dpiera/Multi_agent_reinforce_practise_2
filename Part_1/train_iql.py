...
def train(env, config, output=True):
    ...
    for eps_num in range(config["total_eps"]):
        ...
        while not done:
            agents.schedule_hyperparameters(step_counter, max_steps, method='exponential')
...
if __name__ == "__main__":
    learning_rates_to_test = [0.01, 0.1, 0.5]
    ...
    for lr in learning_rates_to_test:
        ...
        # 1. Update Config with the specific LR for this run
        current_config = CONFIG.copy()
        current_config["lr"] = lr
        
        # 2. Set Seeds (Ensures the 'randomness' is fair for every run)
        ...
        # 4. Train the Agents
        # Note: Make sure your train() function calls schedule_hyperparameters(..., method='exponential')
        # if you want to test your new decay method too!
        means, stds, eval_q, final_q = train(env, current_config)
        visualise_q_convergence(eval_q, env)
        visualise_evaluation_returns(means, stds)

        # 5. Show/Save Results
        print(f"Finished LR={lr}. Please close the plot window to start the next experiment.")
        
        # We add a title to the plot so you know which one it is
        visualise_evaluation_returns(means, stds)
